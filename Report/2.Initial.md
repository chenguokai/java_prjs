...
到这里为止，如果我们不深究细节和算法，fastjson的面貌已经基本呈现在我们面前。从上面的代码设计中可以看出，呈现给用户或可以有效且有必要高度抽象的对象都呈现为抽象类甚至接口的形式，而接口、抽象类、普通类之间的层层继承也充分反应了fastjson设计者对于功能主次、性能高低的良好把握。在每个用于解析较为复杂的对象的类中，往往会相应设计一个对应的Config类以记录其配置信息而非将配置与算法混杂在一起。这充分体现了面向对象的解耦思想。
